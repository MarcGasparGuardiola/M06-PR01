...
function getFavouritePlayers() {
    return getList('favouritePlayers')
}
...
function checkRepeatedPlayer(id) {
    return myTeam.some((val) => val.player.id === Number(id));
}
...
dropZone.addEventListener('drop', (e) => {
    ...
    if (!checkNumberOfPlayers()) {
        showAlert('ERROR: You have the max number of players in the team');
        return;
    }
    ...
    if (!checkPositionOfPlayer(player.statistics[player.statistics.length - 1].games.position)) {
        showAlert(`ERROR: Max number of ${player.statistics[player.statistics.length - 1].games.position}s reached`);
        return;
    }
    ...
    if (!checkRepeatedPlayer(player.player.id)) {
        myTeam.push(player);
        console.log(myTeam);
        //Save player to myTeam localStorage
        saveList('myTeam', myTeam);
        closestDiv.insertAdjacentHTML('beforeEnd', str);
    } else {
        showAlert('ERROR: This player is alredy in the team');
    }
    dropZone.style.borderColor = 'black';
    idDraged = 0;
})
...
function removePlayerFromMyTeam(id) {
    return myTeam.filter((player) => { 
        ...
        console.log(Number(player.player.id));
        console.log(player.player.id !== Number(id))
...
dropZone.addEventListener('click', (e) => {
    ...
    if (e.target.matches('div *')) { 
        ...
        console.log(id);
        myTeam = removePlayerFromMyTeam(id);
        console.log(myTeam);
        saveList('myTeam', myTeam);
